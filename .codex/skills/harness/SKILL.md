...
# Harness — Agent Team & Skill Architect for Codex

Codex CLI와 Codex Desktop에서 사용할 하네스를 구성하고, 각 에이전트의 역할을 정의하며, 에이전트가 사용할 스킬을 생성하는 메타 스킬.

**핵심 원칙:**
1. 에이전트 정의와 스킬을 프로젝트 로컬 파일로 생성한다.
2. **에이전트 팀을 기본 실행 모드로 사용한다.**
3. **프로젝트 안내 파일에 하네스 포인터를 등록한다.** — 새 세션에서 오케스트레이터 스킬이 다시 읽히도록 최소한의 포인터(트리거 규칙 + 변경 이력)만 기록한다.
4. **하네스는 고정물이 아니라 진화하는 시스템이다.** — 매 실행 후 피드백을 반영하고, 에이전트·스킬·프로젝트 안내 파일을 지속 갱신한다.
...
1. `프로젝트/agents/`, `프로젝트/skills/`, `프로젝트/AGENTS.md` 또는 `프로젝트/CODEX.md` 같은 안내 파일을 읽는다
...
3. 기존 에이전트/스킬 목록과 안내 파일 기록을 대조하여 불일치(drift)를 감지한다
...
**모든 에이전트는 반드시 `프로젝트/agents/{name}.md` 파일로 정의한다.** 에이전트 정의 파일 없이 역할을 prompt에 직접 넣는 것은 금지한다. 이유:
...
각 에이전트가 사용할 스킬을 `프로젝트/skills/{name}/SKILL.md`에 생성한다. 상세 작성 가이드는 `references/skill-writing-guide.md` 참조.
...
description은 스킬의 유일한 트리거 메커니즘이다. Codex는 트리거를 보수적으로 판단할 수 있으므로, description을 **적극적("pushy")**으로 작성한다.
...
#### 5-4. 프로젝트 안내 파일에 하네스 포인터 등록

하네스 구성 완료 후, 프로젝트의 `AGENTS.md` 또는 `CODEX.md` 같은 안내 파일에 최소한의 포인터를 등록한다. 이 파일은 새 세션마다 다시 읽히는 프로젝트 안내의 역할을 하므로, 하네스 존재와 트리거 규칙만 기록하면 오케스트레이터 스킬이 나머지를 처리한다.
...
**안내 파일에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세. 이유: 에이전트/스킬 목록은 오케스트레이터 스킬과 프로젝트의 `agents/`, `skills/`에서 관리하므로 중복이다. 디렉토리 구조는 파일 시스템에서 직접 확인 가능하다. 안내 파일은 **포인터(트리거 규칙) + 변경 이력**만 담는다.
...
모든 변경은 안내 파일의 **변경 이력** 테이블에 기록한다 (Phase 5-4 템플릿의 "변경 이력" 섹션과 동일 테이블):
...
- `skills/` 디렉토리 목록과 오케스트레이터 스킬의 스킬 구성 비교 → 불일치 목록 생성
- 감사 결과를 사용자에게 보고한다
...
- [ ] **안내 파일에 하네스 포인터 등록** (트리거 규칙 + 변경 이력)
- [ ] **안내 파일 변경 이력에 에이전트/스킬 추가/삭제/수정 기록**
