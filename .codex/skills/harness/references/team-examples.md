...
> 리서치 에이전트는 `general-purpose` 빌트인 타입을 사용하되, 반드시 `agents/{name}.md` 파일로 정의한다. 파일에는 역할·조사 범위·팀 통신 프로토콜을 명시하여 재사용성과 협업 품질을 보장한다.
...
### 스킬 파일 구조
위치: `프로젝트/skills/{skill-name}/SKILL.md` (프로젝트 레벨)
